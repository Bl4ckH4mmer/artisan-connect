...
export default function AnalyticsWidget({ title, value, icon, trend, color }: AnalyticsWidgetProps) {
    ...
    return (
        <div className={`bg-white rounded-xl shadow-sm p-3 sm:p-6 border-l-4 ${colors.border}`}>
            <div className="flex items-center justify-between mb-2 sm:mb-4">
                <div className={`p-2 sm:p-3 ${colors.bg} rounded-lg`}>
                    <div className={`${colors.text} [&>svg]:w-4 [&>svg]:h-4 sm:[&>svg]:w-6 sm:[&>svg]:h-6`}>
                        {icon}
                    </div>
                </div>
                {trend && (
                    <div className={`flex items-center gap-1 text-xs sm:text-sm font-medium ${trend.isPositive ? 'text-green-600' : 'text-red-600'
                        }`}>
                        {trend.isPositive ? (
